...
function resolveProfile(config, launcherRoot) {
  ...
  let profileId = getSelectedProfileId(config);
  const profile = config.profiles?.[profileId] || {};
  let modpackId = profile.modpackId || profileId;
  let modpack = loadModpack(launcherRoot, modpackId);
  if (!modpack) {
    const fallbackId = modpacks.find((pack) => pack.id === 'bloodpact-26.1.2')?.id || modpacks[0]?.id;
    if (fallbackId && fallbackId !== modpackId) {
      profileId = fallbackId;
      modpackId = fallbackId;
      config.selectedProfile = fallbackId;
      modpack = loadModpack(launcherRoot, fallbackId);
    }
  }
  if (!modpack) {
    modpack = modpacks[0] || readJson(path.join(launcherRoot, 'modpack.json'), {});
  }
